...
The implementation centralizes the current palette in `app/globals.css`: `--background` is `#1e1e2e`, `--surface` is `#181825`, `--surface-raised` is `#313244`, `--text` is `#6c7086`, `--muted` is `#9399b2`, `--subtle` is `#2e303d`, `--accent` is `#a6e3a1`, and `--anchor` is `#ff3030`. These values are intentionally kept in the stylesheet as the current visual direction.
